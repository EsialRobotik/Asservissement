...
{
	lastStatus = 0;
	return liste->enqueue(CMD_GOTO, Utils::mmToUO(odometrie, posXInmm),
			Utils::mmToUO(odometrie, posYInmm));
}

bool CommandManager::addGoToEnchainement(double posXInmm, double posYInmm)
{
	lastStatus = 0;
	return liste->enqueue(CMD_GOTOENCHAIN, Utils::mmToUO(odometrie, posXInmm),
			Utils::mmToUO(odometrie, posYInmm));
}
...
{
	lastStatus = 0;
	return liste->enqueue(CMD_GOTOANGLE, Utils::mmToUO(odometrie, posXInmm),
			Utils::mmToUO(odometrie, posYInmm));
}
...
void CommandManager::perform()
{
	...
	if (emergencyStop)
	{
		...
		{ //On s'assure que la liste des commandes est vide
...
void CommandManager::computeGoTo()
{
	...
	else
	{
		cnsgCtrl->set_angle_consigne(
				Utils::radToUO(odometrie, deltaTheta)
						+ cnsgCtrl->getAccuAngle()); //on se met dans la bonne direction
...
void CommandManager::computeGoToAngle()
{
	...
	// La consigne à atteindre en angle est la somme du deltaTheta en UO et de l'accumulateur du régu
	int64_t consigne_angle = Utils::radToUO(odometrie, deltaTheta)
			+ cnsgCtrl->getAccuAngle();
	cnsgCtrl->set_angle_consigne(consigne_angle);   // On set la consigne
...
void CommandManager::computeEnchainement()
{
	...
	if (deltaDist < Config::enchainThreshold)
	{ // On a le droit de passer à la consigne suivante
		currCMD = nextCMD; // La consigne suivante devient la consigne courante
		nextCMD = liste->dequeue(); // On essaye de récupérer la prochaine consigne
