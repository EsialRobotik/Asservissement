...
void ConsignController::perform()
{
	if (perform_on)
	{
		...
		{
			angle_output = angle_regu.manage(angle_consigne,
					odometrie->getDeltaThetaBrut());
		}
		...
		int32_t VmoteurD = Utils::constrain(dist_output + angle_output,
				Config::V_MAX_NEG_MOTOR, Config::V_MAX_POS_MOTOR);
		int32_t VmoteurG = Utils::constrain(dist_output - angle_output,
				Config::V_MAX_NEG_MOTOR, Config::V_MAX_POS_MOTOR);
